    ...
    <div className="flex min-h-screen items-center justify-center">
      <div className="flex flex-col gap-2">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-start gap-2">
          <textarea {...register("fens", { required: true })} className="text-black" />
          {errors.fens && <span>This field is required</span>}
          <button type="submit" className="rounded bg-amber-500 px-4 py-2 font-bold text-white hover:bg-amber-700">
            Submit
          </button>
        </form>
        {index !== undefined && <Chessboard board={parseFen(fenList[index])} />}
        {fenList.length > 1 && <div className="flex flex-col gap-2">
